...
The datatype of carat is continous and for clarity is categorical therefore the variance between the domify features holds less weight than carat
...
There is high n ( number of obvservations ) low p ( number of features) so overfitting is probably not an issue
...
For this data yes it would be the best fitting model, we are transforming the variables and making them have a linear relationship
...
If carat increases ny p-percent, then price increases by p-percent*1.7 


Interpret b_0, the estimated intercept.

for a 1-carat diamond the price is predictied to be e^8.5
...
y = lm(ln_price ~ diamonds_test$ln_price)

mod_f = lm(ln_price ~ (poly(carat, 3) + poly(x, 3) + poly(y, 3) + poly(z, 3) + poly(depth, 3) + poly(table, 3))*(clarity + cut + color) , diamonds)

y_hat_test = predict(mod_f, diamonds_test)
mean(sum(sqrt(y - y_hat_tes)))
```
